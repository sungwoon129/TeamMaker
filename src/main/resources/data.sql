

INSERT INTO `auction_rule` (`rule_id`,`proceed_way`,`skeleton`) VALUES (1,'NORMAL',5);


INSERT INTO `participant_role` (`participant_role_id`,`role_name`,`point`,`rule_id`) VALUES (1,'ZEUS','700',1);
INSERT INTO `participant_role` (`participant_role_id`,`role_name`,`point`,`rule_id`) VALUES (2,'ONER','750',1);
INSERT INTO `participant_role` (`participant_role_id`,`role_name`,`point`,`rule_id`) VALUES (3,'FAKER','550',1);
INSERT INTO `participant_role` (`participant_role_id`,`role_name`,`point`,`rule_id`) VALUES (4,'GUMAYUSI','600',1);
INSERT INTO `participant_role` (`participant_role_id`,`role_name`,`point`,`rule_id`) VALUES (5,'KERIA','650',1);


INSERT INTO `team_position` (`position_id`,`position_name`,`rule_id`) VALUES (1,'TOP',1);
INSERT INTO `team_position` (`position_id`,`position_name`,`rule_id`) VALUES (2,'JUNGLE',1);
INSERT INTO `team_position` (`position_id`,`position_name`,`rule_id`) VALUES (3,'MID',1);
INSERT INTO `team_position` (`position_id`,`position_name`,`rule_id`) VALUES (4,'AD CARRY',1);
INSERT INTO `team_position` (`position_id`,`position_name`,`rule_id`) VALUES (5,'SUPPORT',1);


INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (1,'KIIN',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (2,'CANYON',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (3,'CHOVY',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (4,'PEYZ',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (5,'LEHENDS',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');

INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (6,'DORAN',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (7,'PEANUT',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (8,'ZEKA',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (9,'VIPER',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (10,'DELIGHT',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');

INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (11,'PERFECT',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (12,'PYOSIK',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (13,'BDD',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (14,'DEFT',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (15,'BERYL',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');

INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (16,'KINGEN',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (17,'LUCID',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (18,'SHOWMAKER',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (19,'AIMING',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');
INSERT INTO `auction_item` (`item_id`,`name`,`rule_id`, `img`) VALUES (20,'KELLIN',1, 'https://image.fmkorea.com/files/attach/new3/20230329/14339012/5169126239/5626726225/a96e8f0a4751850e1c87388575c7a195.jpg');

INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (1,'KIIN',10,'<iframe width="100%" height="100%" src="https://www.youtube.com/embed/d0mRzUIxvpo?autoplay=1&si=6LDBYg5TVGcNE92M" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>',1);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (2,'CANYON',10,'<iframe width="100%" height="100%" src="https://www.youtube.com/embed/pyrQ-iy1i_M?autoplay=1&si=5j0b8WeCAI9UB_WI" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>',2);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (3,'CHOVY',10,'<iframe width="100%" height="100%" src="https://www.youtube.com/embed/d0mRzUIxvpo?autoplay=1&si=6LDBYg5TVGcNE92M" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>',3);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (4,'PEYZ',10,'<iframe width="100%" height="100%" src="https://www.youtube.com/embed/icHgZhiun1c?autoplay=1&si=MWsGpnPfNhu50_DR" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>',4);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (5,'LEHENDS',10,'<iframe width="100%" height="100%" src="https://www.youtube.com/embed/Ag5BuDho8qs?autoplay=1&si=LCIRKYxZDB0CfXHH" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>',5);

INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (6,'DORAN',10,'<iframe width="560" height="315" src="https://www.youtube.com/embed/d0mRzUIxvpo?si=6LDBYg5TVGcNE92M" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>',6);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (7,'PEANUT',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',7);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (8,'ZEKA',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',8);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (9,'VIPER',10,'<iframe width="100%" height="100%" src="https://www.youtube.com/embed/6BJvHEus4hk?autoplay=1&si=C2Ni8AdbgyuneCBR" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>',9);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (10,'DELIGHT',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',10);

INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (11,'PERFECT',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',11);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (12,'PYOSIK',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',12);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (13,'BDD',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',13);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (14,'DEFT',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',14);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (15,'BERYL',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',15);

INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (16,'KINGEN',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',16);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (17,'LUCID',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',17);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (18,'SHOWMAKER',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',18);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (19,'AIMING',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',19);
INSERT INTO `highlight` (`highlight_id`,`name`,`length`,`url`,`item_id`) VALUES (20,'KELLIN',10,'https://youtu.be/t1_FVzv4l-4?si=XsPcnLvDt34yg-KK',20);







